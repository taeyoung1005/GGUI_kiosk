...
> 음성 적응형 키오스크 (OBA Weekend-thon · GGUI 트랙)
...
| 경로 | 트리거 | 동작 |
|------|--------|------|
| **(1) LOCAL (demo main)** | `GGUI_MODE=local`(기본) | 요청만으로 적응형 HTML 을 직접 생성해 이 서버의 `/r/:id` 로 서빙. |
...
cd module-c
npm install
GGUI_MODE=local node server.js  # http://localhost:8002
```

전체 앱 기준 설정은 루트 `.env.example` 을 `.env` 로 복사해 한 곳에서 관리한다.
...
- **GGUI 경로**: D 는 GGUI 응답을 iframe/srcDoc 으로 표시한다. 현재 주문 진행은 D의 내장 상태기계가 담당한다.
...
# 2) Module C 를 ggui 모드로. 루트 .env 에 GGUI_MODE=ggui, OPENAI_API_KEY=sk-...,
#    GGUI_URL=http://localhost:6781 을 둔다.
GGUI_MODE=ggui GGUI_FORCE_CREATE=1 node server.js
```
...
| 키 | 기본값 | 설명 |
|----|--------|------|
| `GGUI_WRAPPER_PORT` | `8002` | 서버 포트(D 의 `VITE_GGUI_URL` 가 가리킴) |
...
| `GGUI_MODEL` | `openai:gpt-5.5-2026-04-23` | GGUI 생성 모델 |
