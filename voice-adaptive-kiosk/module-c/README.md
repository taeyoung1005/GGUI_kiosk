...
> Node(ESM) + Express. **두뇌 겸 렌더 = GGUI 의 BYOK LLM(OpenAI GPT).** EXAONE 없음.
...
| 경로 | 트리거 | 동작 |
|------|--------|------|
| **(1) GGUI (primary)** | `GGUI_MODE=ggui` | GGUI MCP 서버(`@ggui-ai/cli serve`, 기본 6781)의 `ggui_render` 호출. OpenAI BYOK 로 UI 생성. 임베드는 GGUI 의 `/r/<shortCode>`. |
| **(2) LOCAL_FALLBACK** | `GGUI_MODE=local`(기본) **또는** GGUI/키 호출 실패 | 요청만으로 적응형 HTML 을 직접 생성해 이 서버의 `/r/:id` 로 서빙. **키 없이 전체 파이프라인이 돈다.** |

`GGUI_MODE=ggui` 라도 GGUI/OPENAI 가 미가동이면 자동으로 LOCAL 로 폴백한다(응답 헤더 `X-GGUI-Path: local-fallback`).
...
`src/adapt.js` 가 정본. `age_group="50+"` 이면 강도를 한 단계 부드럽게 가중(보조 신호).
...
LOCAL HTML 은 `speechSynthesis`(ko-KR)로 진입 시 1회 음성안내(가능 브라우저), "다시 듣기" 버튼 제공.
...
ggui serve                            # http://127.0.0.1:6781  (/mcp + /r/<shortCode>)
...
`ggui_handshake({intent, blueprintDraft:{contract, variance:{seedPrompt}}})` →
`ggui_render({handshakeId, decision:{kind:"accept"}, props})` →
응답의 `renderId/shortCode` 로 `embed_url = {GGUI_URL}/r/<shortCode>` 구성.
